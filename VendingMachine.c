...
//Wmotor 2 is for PORTC 0xF0
//Wmotor 3 is for PORTC 0x0F
...
void main() {
     ...
     //Keypad
      keyinit();
        ...
        LCD_String_xy(1,0,"Select(1,2,3):");//Start from pos 14
        ...
        {
        LCD_String_xy(1,13,":");//Move cursor back to select
...
void LCD_String_xy (unsigned char row,unsigned char pos,unsigned char *str) /* Send string to LCD
function */
{
    location=0;
    if(row<=1)
